package br.com.paxtecnologia.pma.relatorio.dao;

import br.com.paxtecnologia.pma.relatorio.util.FormataDataUtil;
import br.com.paxtecnologia.pma.relatorio.vo.ParadasPorTipoVO;
import br.com.paxtecnologia.pma.relatorio.vo.UltimoAnoVO;
import java.sql.PreparedStatement;
import java.sql.ResultSet;
import java.sql.SQLException;
import java.util.ArrayList;
import java.util.Calendar;
import java.util.List;

public class ParadasDAO {
	private DataSourcePMA connection;

	public Calendar getDataUltimoPNP(Integer idCliente, String mesRelatorio) {
		java.sql.Date data = null;
		Calendar cal = Calendar.getInstance();
		this.connection = new DataSourcePMA();

		String sql = "SELECT data_ult_parada FROM pmp_sem_parada WHERE cliente_id = ?   and trunc(data_insercao,'MM') = trunc(?,'MM')";

		PreparedStatement pstmt = this.connection.getPreparedStatement(sql);
		try {
			pstmt.setInt(1, idCliente.intValue());
			pstmt.setDate(2, FormataDataUtil.formataDataInicio(mesRelatorio));
		} catch (SQLException e) {
			e.printStackTrace();
		}
		ResultSet rs = this.connection.executaQuery(pstmt);
		try {
			while (rs.next()) {
				data = rs.getDate("data_ult_parada");

				cal.setTimeInMillis(data.getTime());
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}

		this.connection.closeConnection(pstmt);
		return cal;
	}

	public Integer getQtdeParadaEvitadasTotal(Integer idCliente, String mesRelatorio) {
		Integer retorno = null;
		this.connection = new DataSourcePMA();

		String sql = "SELECT qtd_pe FROM pmp_sem_parada WHERE cliente_id = ? and trunc(data_insercao,'MM') = trunc(?,'MM')";
		PreparedStatement pstmt = this.connection.getPreparedStatement(sql);
		try {
			pstmt.setInt(1, idCliente.intValue());
			pstmt.setDate(2, FormataDataUtil.formataDataInicio(mesRelatorio));
		} catch (SQLException e) {
			e.printStackTrace();
		}
		ResultSet rs = this.connection.executaQuery(pstmt);
		try {
			while (rs.next()) {
				retorno = Integer.valueOf(rs.getInt("qtd_pe"));
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
		this.connection.closeConnection(pstmt);
		return retorno;
	}

	public List<ParadasPorTipoVO> getListaParadasPorTipo(Integer idCliente, String mesRelatorio, String tipo) {
		this.connection = new DataSourcePMA();

		String sql = "SELECT c.chamado, to_char(c.data_criacao, 'dd/mm/yyyy') data, to_char(a.data_inicio_parada, 'dd/mm/yyyy') data_parada, round(to_number(a.data_fim_parada - a.data_inicio_parada) * 24,2) segundos_trabalhados, pmp_get_hosts_task(c.task_id) nome_fantasia, c.titulo FROM pmp_task_parada a, pmp_parada b, pmp_task c WHERE a.parada_id = b.parada_id   AND a.task_id = c.task_id   AND c.cliente_id = ?   AND c.data_insercao between ? and ?   AND trunc(c.data_insercao,'MM') = trunc(a.data_inicio_parada,'MM')   AND regexp_like(b.tipo_parada,?) ORDER BY c.data_criacao";

		PreparedStatement pstmt = this.connection.getPreparedStatement(sql);
		try {
			pstmt.setInt(1, idCliente.intValue());
			pstmt.setDate(2, FormataDataUtil.formataAnoInicio(mesRelatorio));
			pstmt.setDate(3, FormataDataUtil.formataDataInicio(mesRelatorio));
			pstmt.setString(4, "^" + tipo + " ");
		} catch (SQLException e) {
			e.printStackTrace();
		}
		ResultSet rs = this.connection.executaQuery(pstmt);
		List<ParadasPorTipoVO> listaParadasPorTipoVO = new ArrayList<ParadasPorTipoVO>();
		try {
			while (rs.next()) {
				ParadasPorTipoVO paradasPorTipoVO = new ParadasPorTipoVO();
				paradasPorTipoVO.setIdchamado(rs.getString("chamado"));
				paradasPorTipoVO.setData(rs.getString("data"));
				paradasPorTipoVO.setDataParada(rs.getString("data_parada"));
				paradasPorTipoVO.setHoras(Double.valueOf(rs.getDouble("segundos_trabalhados")));
				paradasPorTipoVO.setHost(rs.getString("nome_fantasia"));
				paradasPorTipoVO.setTitulo(rs.getString("titulo"));
				listaParadasPorTipoVO.add(paradasPorTipoVO);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
		this.connection.closeConnection(pstmt);
		return listaParadasPorTipoVO;
	}

	public List<UltimoAnoVO> getListaUltimosAnosHoras(Integer idCliente, String tipo, String mesRelatorio) {
		this.connection = new DataSourcePMA();

		String sql = "SELECT to_char(c.data_insercao, 'yyyy') data, round(sum(to_number(a.data_fim_parada - a.data_inicio_parada)) * 24,2) horas_trabalhadas FROM pmp_task_parada a, pmp_parada b, pmp_task c WHERE a.parada_id = b.parada_id   AND a.task_id = c.task_id   AND c.cliente_id = ?   AND regexp_like(b.tipo_parada,?)   AND c.data_insercao <= ?group by to_char(c.data_insercao, 'yyyy') order by 1 asc";

		PreparedStatement pstmt = this.connection.getPreparedStatement(sql);
		try {
			pstmt.setInt(1, idCliente.intValue());
			pstmt.setString(2, "^" + tipo + " ");
			pstmt.setDate(3, FormataDataUtil.formataDataInicio(mesRelatorio));
		} catch (SQLException e) {
			e.printStackTrace();
		}
		ResultSet rs = this.connection.executaQuery(pstmt);
		List<UltimoAnoVO> listaUltimoAnoVO = new ArrayList<UltimoAnoVO>();
		try {
			while (rs.next()) {
				UltimoAnoVO ultimoAnoVO = new UltimoAnoVO();
				ultimoAnoVO.setAno(rs.getString("data"));
				ultimoAnoVO.setHoras(Double.valueOf(rs.getDouble("horas_trabalhadas")));
				listaUltimoAnoVO.add(ultimoAnoVO);
			}
		} catch (SQLException e) {
			e.printStackTrace();
		}
		this.connection.closeConnection(pstmt);
		return listaUltimoAnoVO;
	}
}