...
public class RelatorioDAO {
	...
	public RelatorioVO getRelatorioById(Integer relatorioId, String mesRelatorio) {
		...
		try {
			...
			String sql = "select a.relatorio_id " + ",a.nome " + ",a.display_name " + ",a.projeto_jira_id "
					+ ",a.tipo_relatorio_id " + ",a.titulo_capa " + ",b.logo_str " + ",b.cliente_display_name "
					+ "from rel_relatorios a " + ",rel_projeto_jira b " + "where a.relatorio_id = ? "
	...
	public List<MesRelatorioVO> getListaMes(Integer relatorioId) {
		...
		try {
			PreparedStatement pstmt;
			String sql = "SELECT DISTINCT a.data_insercao " + "FROM pmp_task a, " + "rel_relatorios b "
					+ " WHERE a.cliente_id = b.projeto_jira_id " + "and b.relatorio_id = ? " + "order by 1 desc";
			pstmt = connection.getPreparedStatement(sql);
			try {
				pstmt.setInt(1, relatorioId);
			} catch (SQLException e) {
	...
	public String getLogoStr(Integer relatorioId) {
		...
		try {
			...
			String sql = "select  b.logo_str " + "from rel_relatorios a " + ",rel_projeto_jira b "
					+ "where a.relatorio_id = ? " + "and a.projeto_jira_id = b.projeto_jira_id";
			pstmt = connection.getPreparedStatement(sql);
			try {
				pstmt.setInt(1, relatorioId);
			} catch (SQLException e) {
	...
	public String getClienteDisplayName(Integer relatorioId) {
		...
		try {
			...
			String sql = "select  b.cliente_display_name " + "from rel_relatorios a " + ",rel_projeto_jira b "
					+ "where a.relatorio_id = ? " + "and a.projeto_jira_id = b.projeto_jira_id";
			pstmt = connection.getPreparedStatement(sql);
			try {
				pstmt.setInt(1, relatorioId);
